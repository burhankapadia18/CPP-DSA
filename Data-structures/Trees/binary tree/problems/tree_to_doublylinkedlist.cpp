...
void util(node *root, node* &head, node* &prev, int &f) {
    if(!root)
        return;
    util(root->left,head,prev,f);
    if(f==0) {
        f=1;
        head = root;
        prev = root;
    ...
    util(root->right,head,prev,f);
}
node* BT2DLL(node *root) {
    // segmentation fault error coming
    node *head = NULL;
    node *prev = NULL;
    int f=0;
    util(root,head,prev,f);
    return head;
}
